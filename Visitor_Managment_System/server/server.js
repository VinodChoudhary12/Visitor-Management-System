...
app.post('/request-meeting', async (req, res) => {
    const { name, email, phone, meeting_time, meetingTo, Description, meetingToName } = req.body;

    // SQL query to insert the visitor information into the database
    const sql = 'INSERT INTO visitors (name, email, phone, meeting_time, meetingTo, Description , view_By_Visitor,meetingToName) VALUES (?, ?, ?, ?, ?, ?,1,?)';
    db.query(sql, [name, email, phone, meeting_time, meetingTo, Description, meetingToName], async (err, result) => {
        if (err) {
...
app.get("/my-metting/:name", (req, res) => {
    ...
    const selectSql = `
        SELECT id, name, email, phone, meetingTo, meetingToName,
...
app.get('/bellIcon/:name', (req, res) => {
    const name = req.params.name;
    const selectSql = 'SELECT id, name, email, phone, meetingTo,meetingToName, DATE_FORMAT(meeting_time, "%Y-%m-%d %H:%i:%s") AS meeting_time, entry_time, exit_time, Description, qr_code, status, view_By_user FROM visitors WHERE meetingTo = ? AND view_By_user = false';
    db.query(selectSql, name, (err, result) => {
        if (err) throw err;
        if (result.length > 0) {
...
app.post('/addVisitor', upload.single('image'), async (req, res) => {
    ...
    try {
        ...
        const sql = 'INSERT INTO visitoraccount (name, phone, email, password, image) VALUES (?, ?, ?, ?, ?)';
        db.query(sql, [name, phone, email, hashedPassword, imageBuffer], (err, result) => {
            if (err) return res.status(500).json({ error: err.message });
            res.status(200).json({ message: 'Visitor added successfully' });
        });
    } catch (err) {
...
app.get('/todayMeetings/:email', (req, res) => {
    const email = req.params.email;
    const selectSql = `
        SELECT id, name, email, phone, meetingTo, 
               DATE_FORMAT(meeting_time, "%Y-%m-%d %H:%i:%s") AS meeting_time, 
...
//     const selectSql = "select * from visitors where status ='APPROVED' AND DATE(meeting_time) = CURDATE() ORDER BY meeting_time ASC"
//     try {
...
//                 res.json(result);
//             } else {
//                 res.status(200).json({ message: 'No Mettings Today' });
//             }
//         });
//     } catch (error) {
//         console.log(error);
...
//     // Query to check if email exists in `users` table
...
//             });
//         } else {
//             // If not found in users, check in visitors
...
//                     return res.status(404).json({ error: 'Email not found in users or visitors.' });
//                 }
//             });
//         }
//     });
...
app.post('/upload', upload.single('image'), (req, res) => {
    ...
    db.query(query, [originalname, buffer], (err, results) => {
        if (err) {
